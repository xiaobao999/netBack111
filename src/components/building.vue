          ...
          <el-tabs v-model="activeName" @tab-click="handleTabsClick">
            ...
            <el-tab-pane label="关系信息" name="second">
              <div>
                ...
                <el-table :data="relationTableData" border max-height="650" stripe style="width: 100%">
...
export default {
  ...
  methods: {
    ...
    renderContent(h, { node, data, store }) {
      if(data.children){
          return (<span class="custom-tree-node" style="display: inline-block;width:85%;">
          <span style="display:inline-block;min-height:30px;line-height:30px;">{node.label}</span>
          <span style="float:right">
            <el-button size="mini" type="button" style="padding: 5px 10px;" on-click={ () => this.append(data) }>+</el-button>
            <el-button size="mini" type="button" style="padding: 5px 10px;" on-click={ () => this.remove(node, data) }>-</el-button>
          </span>
        </span>);
      }else{
          return (<span class="custom-tree-node" style="padding: 5px 10px;" style="display: inline-block;width:90%;">
          <span style="display:inline-block;min-height:30px;line-height:30px;">{node.label}</span>
        </span>);
      }
