  ...
  <div class="netstration">
    ...
    <div class="atlas">
      ...
      <div class="atlasshow" v-for="(item,i) in atlaslist" :key="i">
        <h3>{{item.atlasname}}</h3>
        <div>创建人:{{item.atlasfounder}}</div>
        <div>创建时间：{{item.atlastime}}</div>
        <div>所属领域：{{item.atlasfield}}</div>
        <hr />
        <div>
          <el-button size="mini">删除</el-button>
          <el-button size="mini" @click="eject('edit',i)">编辑</el-button>
          <el-button size="mini" @click="datement()">图谱管理</el-button>
        </div>
      </div>
    </div>
    <!-- 编辑添加弹出页面 -->
